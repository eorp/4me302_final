...
function get_rss_feed_as_html($feed_url, $max_item_cnt = 10, $show_date = true, $show_description = true, $max_words = 0, $cache_timeout = 7200, $cache_prefix = "/tmp/rss2html-")
{
    ...
    for ($x=0;$x<$max_item_cnt;$x++) {
        ...
        $result .= '<div class="feed-title"><strong><a href="'.$link.'" title="'.$title.'" target="_blank">'.$title.'</a></strong></div>';
        if ($show_date) {
...
//this function takes rss feed url, maximum number of items to be shown, variables that indicate whether date and description should be shown, and maximum number of words to be displayed in the articles preview as parameters, and outputs rss feed 
